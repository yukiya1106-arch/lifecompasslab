  ...
  try{
    ...
    const [encoded,rawEnhancement]=await Promise.all([
      Promise.all(names.map(async name=>{
        const response=await fetch(name+'?v=18',{cache:'no-store'});
        if(!response.ok)throw new Error(name);
        return response.text();
      })).then(parts=>parts.join('')),
      fetch('./enhancements-v9.js?v=18',{cache:'no-store'}).then(response=>{
    ...
    const assetUrl=new URL('./asset-inline-v16.html?v=18',window.location.href).href;
    ...
    frame.addEventListener('load',()=>{
      try{
        frame.contentWindow.eval(enhancement+'\n//# sourceURL=compass-tools-preview-enhancements-v18.js');
        frame.contentDocument.documentElement.dataset.compassToolsPreview='v18';
