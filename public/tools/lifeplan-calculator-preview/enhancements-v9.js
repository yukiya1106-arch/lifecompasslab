...
function ensureAssetFrame(){if(assetHost&&assetFrame)return;assetHost=document.createElement('div');assetHost.id='lc-asset-embedded-host';assetFrame=document.createElement('iframe');assetFrame.id='lc-asset-embedded-frame';assetFrame.title='資産運用4モード';assetFrame.scrolling='no';assetFrame.src=ASSET_URL;assetHost.appendChild(assetFrame);assetFrame.addEventListener('load',()=>requestAnimationFrame(()=>resizeAsset()))}
...
function refresh(){improveDetails();const onAsset=norm(document.body.innerText).includes(norm('資産形成を試算する'));if(onAsset)mountAsset();else unmountAsset()}
