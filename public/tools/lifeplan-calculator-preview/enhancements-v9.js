...
function activateLiving(){const target=[...document.querySelectorAll('button,a,[role="button"]')].find(node=>norm(node.textContent).endsWith(norm('生活費逆算')));if(target)target.click()}
function closeAsset(returnToCalculator=false){if(overlay)overlay.style.display='none';suppressed=true;if(returnToCalculator)setTimeout(activateLiving,0)}
...
window.addEventListener('message',event=>{if(event.data==='lc-close-asset-v9')closeAsset(true)});
